...
class KioscoApp(tk.Tk):
    ...
    def show_admin_stock_frame(self):
        ...
        def descontar_unidad_mano():
            ...
            p_id = int(item[0]) # Obtenemos el ID numérico real
            stock_actual = int(str(item[2]).replace(' u.', ''))
            
            if stock_actual > 0:
                # CORREGIDO: Ahora llamamos a una consulta UPDATE por ID en lugar de insertar duplicados
                # Si tu database.py requiere (nombre, stock, precio, cat), asegúrate de que use un query UPDATE.
                # Aquí llamamos a la función de actualización específica para evitar clones:
                try:
                    db.actualizar_stock_producto(p_id, stock_actual - 1)
                except AttributeError:
                    # Alternativa por si tu db.py maneja la edición con agregar_producto internamente
                    # pero pasando el ID original. Si da error, usamos la estructura clásica adaptada:
                    db.agregar_producto(item[1], stock_actual - 1, float(str(item[3]).replace('$', '')), item[4])
                self.refrescar_stock_admin()
            else:
    ...
    def refrescar_pedidos_admin(self):
        ...
        for r in db.obtener_pedidos_agrupados():
            ...
            # FILTRO ACTIVO: Si el pedido ya se completó o rechazó, no se muestra en la cola de pendientes del admin
            if estado in ["Terminado", "Rechazado"]:
                continue
